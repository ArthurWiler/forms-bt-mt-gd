  ...
  const render = () => {
    ...
    [...select.options]
      ...
      .forEach((o) => {
        ...
        btn.setAttribute(
          "aria-checked",
          select.value === o.value ? "true" : "false",
        );
...
function onCoord() {
  ...
  if (!isNaN(lat) && !isNaN(lon)) {
    ...
    $("[data-k=utm]").value =
      `${u.zona}${_utmBandLetter(lat)} E:${u.easting} N:${u.northing}`;
  }
  ...
  if (
    !isNaN(lat) &&
    !isNaN(lon) &&
    _nDig(state.latitude) >= 5 &&
    _nDig(state.longitude) >= 5
  )
...
async function consultarRestricaoAmbientalLote(lat, lng) {
  ...
  try {
    ...
    renderLicencaAmbiental(
      typeof detalhesRestricoes === "function" ? detalhesRestricoes(res) : null,
    );
  } catch (_) {
    _loteLastRestrKey = "";
  }
...
function onSolicitante() {
  ...
  box.innerHTML = poderPublico
    ? alertHTML(
        "warn",
        "Solicitação pelo Poder Público: anexe o ato administrativo municipal (lei, decreto ou instrumento equivalente) que autoriza o empreendimento.",
      )
    : "";
...
function renderPreview() {
  const totalLotes =
    (parseInt(state.lote_400) || 0) +
    (parseInt(state.lote_400_600) || 0) +
    (parseInt(state.lote_600) || 0);
  const secoes = [
    pvSecao(
      "Dados para contato",
